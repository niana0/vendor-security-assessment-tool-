...
## ⚠️ Action Required: Web Search Needed
...
def process_documents(vendor_files, questionnaire_file, vendor_metadata, progress_bar, status_text):
    ...
    try:
        ...
        if has_documents:
            ...
            status_text.text("🔍 Extracting security evidence from documents...")
        ...
        status_text.text("🌐 Searching public security information...")
        ...
        if vendor_name:
            try:
                status_text.text(f"🔍 Searching web for {vendor_name} security information...")
                ...
                st.warning(f"⚠️ Web search encountered an issue: {str(e)}")
        ...
        status_text.text("📊 Extracting vendor overview...")
        ...
        if questionnaire_path:
            status_text.text("📋 Mapping evidence to questionnaire...")
...
def display_results(results):
    ...
    if assessment_mode == 'web_search_only':
        st.info("🌐 **Assessment Mode:** Public Information Only - No documents provided")
    elif assessment_mode == 'hybrid':
        st.success("📊 **Assessment Mode:** Hybrid Analysis (Documents + Public Information)")
    ...
    if results.get('web_search_error'):
        st.error(f"⚠️ {results['web_search_error']}")
    ...
    if results.get('web_evidence_count', 0) > 0 or results.get('web_incidents_count', 0) > 0:
        ...
        if results.get('web_search_results'):
            ...
            if web_results.get('incidents'):
                with st.expander(f"🚨 View {len(web_results['incidents'])} Security Incidents"):
            ...
            if web_results.get('controls'):
                with st.expander(f"✅ View {len(web_results['controls'])} Security Controls"):
    ...
    if risk.get('threat_model'):
        st.subheader("🎯 Threat Modeling (STRIDE Framework)")
        ...
        if threats:
            ...
            for category, cat_threats in threat_categories.items():
                with st.expander(f"🔍 {category} ({len(cat_threats)} threat(s))"):
...
def main():
    ...
    if not st.session_state.processed:
        ...
        with col1:
            ...
            if vendor_files:
                st.success(f"✅ {len(vendor_files)} file(s) uploaded")
                for vf in vendor_files:
        ...
        if metadata_complete:
            ...
            st.markdown("### 📋 **How This Works (2 Simple Steps)**")

            col1, col2 = st.columns(2)

            with col1:
                st.markdown("""
                **Step 1: Click Button Below** ⬇️
            ...
            with col2:
                st.markdown("""
                **Step 2: Request Search from Claude** 💬
            ...
            if has_documents:
                st.success("📊 **Mode:** Hybrid - Documents + Public Info (documents prioritized)")
            else:
                st.info("🌐 **Mode:** Public Information Analysis")

        # Process button - enabled if vendor name provided
        process_button = st.button(
            "🎯 Generate Risk Assessment (Step 1 of 2)",
            type="primary",
        ...
        if not metadata_complete:
            st.warning("⚠️ Please provide the Vendor Name to proceed.")
